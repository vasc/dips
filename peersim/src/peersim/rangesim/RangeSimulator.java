...
private void parseRanges()
{
	...
	if (Configuration.contains(Simulator.getSimulator().PAR_EXPS) || ranges.length == 0) {
		pars = new String[ranges.length + 1];
		values = new String[ranges.length + 1][];
		pars[0] = "EXP";
		values[0] = StringListParser.parseList("1:"
				+ Configuration.getInt(Simulator.getSimulator().PAR_EXPS, 1));
		start = 1;
	} else {
...
private void doExperiments(String[] args)
{
	...
	list.add(Simulator.getSimulator().PAR_EXPS+"=1");

	// Activate redirection to separate stdout from stderr
	list.add(Simulator.getSimulator().PAR_REDIRECT+"="+TaggedOutputStream.class.getCanonicalName());
	...
	while (idx[0] < values[0].length) {
		...
		list.set(startlog, Simulator.getSimulator().PAR_REDIRECT+"."+
